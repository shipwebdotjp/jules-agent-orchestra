...
class JulesTUI(App):
    ...
    def action_feedback(self) -> None:
        ...
        def get_feedback(feedback: str):
            if feedback:
                def do_feedback():
                    # TUI implementation for now will be simple feedback sending
                    # instead of interactive loop
                    try:
                        self.client.send_message(task.jules.session_name, feedback)
                        ...
                        self.notify(f"Error: {e}", variant="error")
                    self.call_from_thread(self.refresh_list)
    ...
    def action_send_msg(self) -> None:
        ...
        def send_msg(msg: str):
            if msg:
                def do_send():
                    service = SendService(self.state, self.client, self.github_client, self.cwd)
                    options = SendOptions(run=run, task=task, message=msg, output_func=self.notify)
                    result = service.execute(options)
                    self.notify(result.message or "Message sent")
                    self.call_from_thread(self.refresh_list)
